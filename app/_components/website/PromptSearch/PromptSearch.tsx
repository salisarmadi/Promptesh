...
import { Search, X, ArrowLeft } from "@/app/_components/ui/Icons";
...
export function PromptSearch({
  ...
  return (
    /**
     * action مسیرِ خودِ دسته است، پس در حالتِ بی‌جاوااسکریپت هم فرم مستقیم به
     * «/category/portrait?q=…» می‌رود و نه به «/?category=portrait&q=…» که یک
     * ریدایرکتِ اضافه می‌خواست.
     *
     * ⚠️ اینجا دیگر input مخفیِ category نیست و نباید برگردد: فرمِ GET بومی
     * پارامترهایش را *جایگزینِ* کوئریِ action می‌کند، ولی مسیر را دست نمی‌زند —
     * پس دسته در مسیر امن است و یک فیلدِ مخفی فقط «?category=» را دوباره
     * می‌ساخت، یعنی همان شکلی که برای گریز از آن این مسیر ساخته شد.
     *
     * page عمداً منتقل نمی‌شود: جستجوی تازه از صفحه‌ی یک شروع می‌شود.
     */
    <form onSubmit={onSubmit} action={galleryHref({ category })} method="get" role="search">

 ...
 *
 * ساختِ ظاهری عیناً همان حالتِ خالیِ گالری است (NoResults در گالری): یک آدرسِ
 * اشتباه هم از دیدِ کاربر همان تجربه‌ی «چیزی که می‌خواستم اینجا نبود» است، پس
 * دلیلی ندارد دو زبانِ بصریِ متفاوت داشته باشند.
 *
 * ── چرا <SiteChrome> را خودش صدا می‌زند ──
 * این فایل باید در ریشه‌ی app/ بماند: مستندِ Next صریح است که root not-found
 * آدرس‌های بی‌تطابقِ کلِ برنامه را می‌گیرد. یعنی نمی‌شود بردش داخلِ گروهِ
 * (public). و چون بیرونِ آن گروه است، هدر و فوتر را به ارث نمی‌برد — پس اینجا
 * دستی گذاشته شده. (قبلاً پوسته در layout ریشه بود و این کار لازم نبود.)
 *
 ...
 * پیش‌فرضِ layout می‌ماند که برای صفحه‌ی نیافته دقیق نیست. اگر جای دیگری هم به
 * notFound() رسید، اینجا یک generateMetadata با عنوانِ خودش بگذار.
 */
